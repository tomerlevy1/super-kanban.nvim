...
function M:setup_win(ctx, opts)
  ...
  self.win = Snacks.win({
    ...
    show = false,
    focusable = true,
    bo = { modifiable = false, filetype = constants.board.filetype },
    on_win = function()
...
local function _build_winbar_format_str(conf)
  ...
  local left_scroll_format = with_separator(' ' .. icons.arrow_left .. ' %s', {
    left_sep = '',
    right_sep = right_sep,
    text_hl = '%%#SuperKanbanBoardScrollInfo#',
    sep_hl = '%%#SuperKanbanBoardScrollInfoEdge#',
  })
  ...
  local default_hl = '%%#SuperKanbanBoardWinbar#'

  -- '%%#SuperKanbanBoardToolbar#󰐙  󱈎  󰧮    󰏌 ',
